...
namespace gsmt  {
  ...
  void MemIO::take_buffer ( mmdb::pstr * buf, int * length )  {
  ...
  mmdb::bvector b1;
    if (buffer_length+length>=alloc_length)  {
      ...
      memmove ( b1,buffer,buffer_length );
      mmdb::FreeVectorMemory ( buffer,0 );
      buffer = b1;
    }
    memmove ( &(buffer[buffer_length]),src,length );
  ...
  bool MemIO::read_buffer ( void * dest, const int length,
    ...
    if (buffer_pos+length<=buffer_length)  {
      memmove ( dest,&(buffer[buffer_pos]),length );
      buffer_pos += length;
      return true;
    }
    if (Ok) *Ok = false;
